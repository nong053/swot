    ...
    if ($result_check->num_rows > 0) {
        ...
        if ($conn->query($sql_insert) === TRUE) {
           ...
           //task_cate_ex
            ...
            //setting_ex
            $sql_copy_setting_to_setting_ex ="
            INSERT INTO 
            setting_ex 
            (mc_id,s_code,title,detail,period,period_unit,period_minute,created_date,updated_date)
            SELECT 
            $mc_id,s_code,title,detail,period,period_unit,period_minute,now(),now()
            FROM setting
            WHERE uu_id = '$_REQUEST[uuid]'";

            if ($conn->query($sql_copy_setting_to_setting_ex) === TRUE) {
                $checkError=true;
            }else{
                echo "Error insert: " . $sql_copy_setting_to_setting_ex . "<br>" . $conn->error;
                $checkError=false;
            }
        ...
        *
        ...
        $sqlSetting = "SELECT 
        *
        FROM setting_ex where mc_id='$_REQUEST[mc_id]'";
        $dataSettingArray = array();
        $resultSetting = $conn->query($sqlSetting);
        ...
        echo "[{\"status\":\"200\",
            ...
            \"dataSetting\":".json_encode($dataSettingArray)."
        }]";
        ...
        $SettingLength=sizeof($_REQUEST['dataSetting']);
        ...
        $setting_delete = "DELETE FROM setting WHERE  uu_id='$_REQUEST[uuid]'";
        if ($conn->query($setting_delete) === TRUE) {
            $checkError=true;
        }else{
            $checkError=false;
            echo "Error deleting: " . $setting_delete . "<br>" . $conn->error;
        }
        ...
        for ($i = 0; $i < $SettingLength; $i++) {
            ...
            (
            '".$_REQUEST['uuid']."',
            '".$_REQUEST['dataSetting'][$i]['title']."',
            '".$_REQUEST['dataSetting'][$i]['detail']."',
            '".$_REQUEST['dataSetting'][$i]['period']."',
            '".$_REQUEST['dataSetting'][$i]['period_unit']."',
            '".$_REQUEST['dataSetting'][$i]['period_minute']."',
            '".$_REQUEST['dataSetting'][$i]['s_code']."',
            now(),
            now()
            )
            ";

            if ($conn->query($sql_insert_setting) === TRUE) {
                $checkError=true;
            }else{
                echo "Error insert: " . $sql_insert_setting . "<br>" . $conn->error;
                $checkError=false;
            } 
        ...
        $sql_setting_delete = "DELETE FROM setting WHERE  uu_id='$_REQUEST[uuid]'";
        if ($conn->query($sql_setting_delete) === TRUE) {
            $checkError=true;
        }else{
            $checkError=false;
            echo "Error deleting task: " . $conn->error."<br>";
       ...
       //load_from_setting_ex
       $sql_load_from_setting_ex ="
       INSERT INTO 
       setting
       (uu_id,s_code, title, detail,period,period_unit,period_minute,created_date,updated_date)
       SELECT 
       '$_REQUEST[uuid]',s_code, title, detail,period,period_unit,period_minute,now(),now()
       FROM setting_ex
       WHERE mc_id = '$_REQUEST[mc_id]'";

       if ($conn->query($sql_load_from_setting_ex) === TRUE) {
           $checkError=true;
       }else{
           echo "Error insert: " . $sql_load_from_setting_ex . "<br>" . $conn->error;
           $checkError=false;
       }
